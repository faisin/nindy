...
# Mengambil daftar user dari tag inbounds vmess yang sesuai dengan skrip add vmess
users=$(jq -r '.inbounds[] | select(.tag=="vmess-ws-tls" or .tag=="vmess-grpc") | .settings.clients[].email' "$CONFIG" 2>/dev/null)

if [[ -z "$users" ]]; then
    echo -e "${YELLOW}Tidak ada pengguna VMess yang terdaftar.${NC}"
else

#!/bin/bash
# Perpanjang akun Trojan - by znandev
set -e
...
# Ambil user dari tag inbounds trojan yang sesuai
users=$(jq -r '.inbounds[] | select(.tag=="trojan-ws-tls" or .tag=="trojan-grpc") | .settings.clients[].email' "$CONFIG" 2>/dev/null | sort -u)
...
CLIENT_EXISTS=$(jq -r '.inbounds[] | select(.tag=="trojan-ws-tls" or .tag=="trojan-grpc") | .settings.clients[]?.email' "$CONFIG" 2>/dev/null | grep -w "$user" | wc -l)
...
# Cek data di database trojan.db jika ada, jika tidak buat estimasi dari hari ini
if [ -f "$DB_FILE" ] && grep -q "^$user " "$DB_FILE"; then
    ...
    new_exp=$(date -d "+$tambah days" +%Y-%m-%d)
    uuid=$(jq -r --arg user "$user" '.inbounds[] | select(.tag=="trojan-ws-tls" or .tag=="trojan-grpc") | .settings.clients[]? | select(.email == $user) | .password' "$CONFIG" | head -n 1)
    mkdir -p /etc/xray
